...
func TestOpenAICompleteWithTools_CapturesPromptCacheUsage(t *testing.T) {
	...
	result, err := provider.CompleteWithTools(context.Background(), []ChatMessage{{Role: "user", Content: "play"}}, GetToolSchemas("playing"))
	if err != nil {
		t.Fatalf("CompleteWithTools error: %v", err)
	}
	if result.PromptCacheHitTokens != 80 {
		t.Fatalf("PromptCacheHitTokens = %d, want 80", result.PromptCacheHitTokens)
	}
	if result.PromptCacheMissTokens != 40 {
		t.Fatalf("PromptCacheMissTokens = %d, want 40", result.PromptCacheMissTokens)
	}
	if !strings.Contains(result.RawResponseJSON, "prompt_cache_hit_tokens") {
		t.Fatalf("raw response should include provider usage payload, got %s", result.RawResponseJSON)
	}
}
